...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function updateChart() {
        ...
        try {
            ...
            myChart.options.plugins.title.text = `Count of ${chartState.model.charAt(0).toUpperCase() + chartState.model.slice(1)}s by ${currentLevel.label}`;
            myChart.update();
    ...
    function updateBreadcrumbs() {
        ...
        chartState.drilldownStack.forEach((level, index) => {
            ...
            breadcrumb.textContent = level.parentLabel ? `${level.parentLabel} > ${level.label}` : level.label;
    ...
    recipesBtn.addEventListener('click', () => {
        ...
        recipesBtn.classList.replace('btn-outline-primary', 'btn-primary');
        slidesBtn.classList.replace('btn-primary', 'btn-outline-primary');
        resetToTopLevel('subject', 'Subjects');
    });

    slidesBtn.addEventListener('click', () => {
        // --- THIS IS THE CORRECTED PART ---
        if (chartState.model === 'slide') return;
        chartState.model = 'slide'; // Use 'slide' to match the backend API
        // --- END CORRECTION ---
        slidesBtn.classList.replace('btn-outline-primary', 'btn-primary');
        recipesBtn.classList.replace('btn-primary', 'btn-outline-primary');
        resetToTopLevel('subject', 'Subjects');
    });
